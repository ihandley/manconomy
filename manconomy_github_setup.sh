...
get_milestone_number() {
  local title="$1"
  api GET "/repos/$GITHUB_OWNER/$GITHUB_REPO/milestones?state=all&per_page=100" | python3 - <<'PY' "$title"
import json, sys
wanted = sys.argv[1]
...
issue_exists() {
  local title="$1"
  api GET "/repos/$GITHUB_OWNER/$GITHUB_REPO/issues?state=all&per_page=100" | python3 - <<'PY' "$title"
import json, sys
wanted = sys.argv[1]
items = json.load(sys.stdin)
for item in items:
